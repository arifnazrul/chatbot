...
// "sign on messages" that the bot can you use to start the "chating session".
//
...
void chatterbot6::preprocess_input()
{
...
bool chatterbot6::user_repeat() const
{
...
bool chatterbot6::similar_input() const
{
...
void chatterbot6::select_response()
{
	if(bot_understand())
	{
...
void chatterbot6::handle_repetition()
{
	if(response_list.size() > 0)
	{
...
void chatterbot6::handle_user_repetition()
{
	if(same_input())
	{
...
void chatterbot6::handle_event(std::string str)
{
	save_prev_event();
	set_event(str);
...
int main()
{
	std::cout << "Chatterbot 6.0" << std::endl;

	try
	{
		chatterbot6 bot("Chatterbot5");
